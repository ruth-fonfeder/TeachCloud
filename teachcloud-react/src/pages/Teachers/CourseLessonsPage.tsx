...
const CourseLessonsPage = () => {
  ...
  console.log("🔍 useParams:", { courseId, groupId });
  ...
  useEffect(() => {
    ...
    if (groupId) {
      ...
      if (!isNaN(groupNum)) {
        console.log("✅ groupId מה־URL:", groupNum);
        setStudyGroupId(groupNum);
      }
    }
  
    // setStudyGroupId(Number(groupId)); // 👈 קובע ברגע שהעמוד עולה
    ...
    const fetchGroups = async () => {
      try {
        ...
        if (!groupId && data.length > 0) {
          console.log("💡 לא הגיע groupId מה־URL – לוקחים את הראשון מה־API");
